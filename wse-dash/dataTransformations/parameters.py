...
class Parameters:
    ...
    def __init__(self, abbreviations_of_companies):
        ...
        if len(self.abbreviations_of_companies) > 0: # if more than one company selected
            for i in self.abbreviations_of_companies:
                self.source_df = pd.read_csv(os.path.join(path,r'%s.txt' % i.lower()),
                                             delimiter=',',
                                             index_col=[3]) # important
                self.source_df.index = pd.to_datetime(self.source_df.index)
                self.list_source_df.append(self.source_df)

        else:
            self.source_df = pd.read_csv(os.path.join(path,r'%s.txt' % abbreviations_of_companies[0].lower()),
                                         delimiter=',',
                                         index_col=[3]) # index is important here
            self.source_df.index = pd.to_datetime(self.source_df.index)
            self.list_source_df.append(self.source_df)
    ...
    def daily_movement(self):
        ...
        for i in self.abbreviations_of_companies:
            open_price = database['<OPEN>_%s' % i]
            close_price = database['<CLOSE>_%s' % i]
    ...
    def low_price(self):
        ...
        for i in self.abbreviations_of_companies:
            column_headlines.append('<LOW>_%s' % i)
    ...
    def open_price(self):
        ...
        for i in self.abbreviations_of_companies:
            column_headlines.append('<OPEN>_%s' % i)
    ...
    def close_price(self):
        ...
        for i in self.abbreviations_of_companies:
            column_headlines.append('<CLOSE>_%s' % i)
    ...
    #     '''
    #
    #     database = pd.read_csv(os.path.join('/Users/michalozieblo/Downloads/WSE-dashboard/wse-dash/databases/',
    #                                         r'database.csv'), low_memory=False) # sprawdz to wzgledem tego co sie dzieje doslownie funkcje wyzej
    #
    #     for i in self.abbreviations_of_companies:
    #         open_price = database['<OPEN>_%s' % i]
    #         close_price = pd.Series(database['<CLOSE>_%s' % i])
    #         open_price_subtract = open_price.sub(close_price)
